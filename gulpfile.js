...
gulp.task("pug", function() {
  return gulp
    .src([inputDir + "pug/*.pug", "!" + inputDir + "pug/_*.pug"])
    .pipe(plumber())
...
gulp.task("img:sync", function() {
  return gulp
    .src("")
    .pipe(plumber())
    .pipe(dirSync(inputDir + "i/", outputDir + "i/", { printSummary: true }))
    .pipe(browserSync.stream());
});
...
gulp.task("fonts:sync", function() {
  return gulp
    ...
    .pipe(plumber())
    .pipe(
      dirSync(inputDir + "fonts/", outputDir + "fonts/", { printSummary: true })
    )
    .pipe(browserSync.stream());
});
          ...
          symbol: {
            ...
            render: {
              scss: {
                ...
                template:
                  inputDir + "/style/abstracts/_svg-sprite-template.scss"
              }
            }
          }
        }
...
gulp.task("watch", function() {
  gulp.watch(inputDir + "pug/**/*.pug", ["pug"]);
...
gulp.task("img:prod", function() {
  return gulp
    .src([inputDir + "i/**/*", "!" + inputDir + "i/phantom/**"])
    .pipe(
      imagemin({
        progressive: true,
...
gulp.task("js:prod", function() {
  return gulp
    .src(inputDir + "js/{main,font-loader}.js")
    .pipe(rigger())
    .pipe(uglify())
    .pipe(gulp.dest(outputDir + "js/"));
});
...
gulp.task("prod", ["clean"], function() {
  gulp.start("pug", "style:prod", "js:prod", "img:prod", "svgSprite");
});
